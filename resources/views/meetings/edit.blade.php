    ...
    <div class="py-6">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow-sm sm:rounded-lg p-6">
                <h2 class="text-xl font-bold mb-6 text-gray-800">Kemaskini Mesyuarat</h2>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                        ...
                        <div>
                            ...
                            <select name="title" class="w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200" required>
                                <option value="PRA-JTICTNS" {{ $meeting->title == 'PRA-JTICTNS' ? 'selected' : '' }}>PRA-JTICTNS</option>
                                <option value="JTICTNS" {{ $meeting->title == 'JTICTNS' ? 'selected' : '' }}>JTICTNS</option>
                                <option value="JPICTNS" {{ $meeting->title == 'JPICTNS' ? 'selected' : '' }}>JPICTNS</option>
                            </select>
                        </div>
                            ...
                            <div class="w-1/2">
                                ...
                                <select name="meeting_number" class="w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200" required>
                                    @foreach([1,2,3,4,5,6] as $num)
                                        <option value="{{ $num }}" {{ $meeting->meeting_number == $num ? 'selected' : '' }}>{{ $num }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="w-1/2">
                                <label class="block font-medium text-sm text-gray-700 mb-1">Tahun</label>
                                <input type="number" name="year" value="{{ $meeting->year }}" class="w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200" required>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <input type="date" name="date" value="{{ $meeting->date->format('Y-m-d') }}" class="w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200" required>
                        </div>

                        <div>
                            <label class="block font-medium text-sm text-gray-700 mb-1">Masa</label>
                            <input type="time" name="time" value="{{ \Carbon\Carbon::parse($meeting->time)->format('H:i') }}" class="w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200" required>
                        </div>

                        <!-- Tempat -->
                        <div class="md:col-span-2">
                            <label class="block font-medium text-sm text-gray-700 mb-1">Tempat</label>
                            <input type="text" name="venue" value="{{ $meeting->venue }}" class="w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200" required>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="flex items-center space-x-4">
                                <label class="inline-flex items-center">
                                    <input type="radio" name="status" value="Aktif" class="form-radio text-indigo-600" {{ $meeting->status == 'Aktif' ? 'checked' : '' }} required>
                                    <span class="ml-2">Aktif</span>
                                </label>
                                <label class="inline-flex items-center">
                                    <input type="radio" name="status" value="Tidak Aktif" class="form-radio text-indigo-600" {{ $meeting->status == 'Tidak Aktif' ? 'checked' : '' }}>
                        ...
                        <div>
                            ...
                            <input type="date" name="project_update_deadline" value="{{ $meeting->project_update_deadline->format('Y-m-d') }}" class="w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200" required>
                        </div>

                        <!-- Minit Mesyuarat Lepas -->
                        <div class="md:col-span-2">
                            <label class="block font-medium text-sm text-gray-700 mb-1">Minit Mesyuarat Lepas</label>
                            @if($meeting->minutes_file)
                                <div class="text-sm text-blue-600 mb-2">
                                    <a href="{{ asset('storage/' . $meeting->minutes_file) }}" target="_blank" class="underline">Lihat Fail Semasa</a>
                                </div>
                            @endif
                        ...
                        <div class="md:col-span-2">
                            ...
                            <input id="agenda" type="hidden" name="agenda" value="{{ $meeting->agenda }}">
